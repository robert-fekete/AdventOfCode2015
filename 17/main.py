...
def solve_first(input_string, amount):
    numbers = map(int, input_string.strip().split('\n'))

    return solve_rec(amount, 0, numbers)


def solve_rec_min(target_amount, current_amount, current_containers, min_containers, number_of_solutions,
    ...
    for container in remaining_containers[:]:
        remaining_containers.remove(container)
        min_containers, number_of_solutions = solve_rec_min(target_amount, current_amount + container,
                                                            current_containers + 1, min_containers, number_of_solutions,
                                                            remaining_containers[:])

    return min_containers, number_of_solutions


def solve_second(input_string, amount):
    numbers = map(int, input_string.strip().split('\n'))
